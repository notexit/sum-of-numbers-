...
begin
  ...
  for i:=1 to n do                                           //Створюємо цикл для заповнення по вертикалі
     for j:=1 to m do                                        //Створюємо цикл для заповнення по горизонталі
        begin
          ...
          if (j mod 2=0) then                                 //Перевіряємо, чи стопчик має парне число
             sums:= sums+mas[i,j]                             //При істиності додаємо число до sums
          Else begin
             sums_odd:= sums_odd+mas[i,j];                    //При хибності додаємо число до sums_odd
             end;
          if mas[i,j] mod 2=0 then                             // Перевірка на парність числа в комірці
             even_numbers:= even_numbers+mas[i,j]              //Якщо істино то додаємо до even_numbers
          Else begin
               no_even_numbers:=no_even_numbers+mas[i,j];      //Інакше додаємо до no_even_numbers
          end;
        end;
    ...
    label9.Caption:=IntToStr(no_even_numbers);                //Відображаємо суму всіх не парних стопчиків
    label12.Caption:=IntToStr(sums_odd);                      //Відображаємо суму всіх не парних чисел
        end;
